...
> 2026-08-19 实现偏差说明：AgentDefinition 与 Skill 服务层业务失败现已统一为 HTTP 200 + `ServiceResult<T>`，通过 `Success=false`、`Status=500` 和 `Message` 表达，不再向 API 消费者返回这两条管理链路的专用 `ErrorCode`。Agent Create/Import 成功保留 HTTP 201，Skill Create 成功返回 HTTP 200；Agent 导出和 Skill 文件内容读取保留原响应。下文涉及这两条管理链路专用错误码和 HTTP 4xx 的内容仅作为历史设计记录，其他模块仍以当前代码为准。
