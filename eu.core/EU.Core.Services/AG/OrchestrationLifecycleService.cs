    ...
    IAgentDefinitionCatalog agents) : BaseServices, IOrchestrationLifecycleService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (existing.Status is OrchestrationStatus.Archived ||
            command.Status is OrchestrationStatus.Archived)
            return Failure(
                OrchestrationErrorCodes.LifecycleTransitionInvalid,
                "Use the archive operation to archive or restore an orchestration.");
    ...
    public async Task<ServiceResult<OrchestrationDefinition>> PublishAsync(PublishOrchestrationCommand command, CancellationToken cancellationToken = default)
    {
        ...
        if (existing.Status is OrchestrationStatus.Archived)
            return Failure(
                OrchestrationErrorCodes.LifecycleTransitionInvalid,
                "An archived orchestration must be restored before it can be published.");
    ...
    public async Task<IReadOnlyList<OrchestrationListItem>> ListAsync(OrchestrationStatus? status = null, CancellationToken cancellationToken = default) =>
        OrchestrationContractCloner.ReadOnly((await repository.ListAsync(cancellationToken))
            .Where(value => status.HasValue
                ...
                : value.Status is not OrchestrationStatus.Archived)
            .Select(value =>
        ...
        CancellationToken cancellationToken = default)
    {
        OrchestrationDefinition? existing = await repository.GetByIdAsync(command.Id, cancellationToken);
        if (existing is null)
            return Failure(OrchestrationErrorCodes.NotFound, "The orchestration was not found.");
        if (existing.LogicalRevision != command.ExpectedLogicalRevision) return Conflict();
        if (command.Archived && existing.Status is not OrchestrationStatus.Disabled)
            return Failure(
                OrchestrationErrorCodes.LifecycleTransitionInvalid,
                "An orchestration must be disabled before it can be archived.");
        if (!command.Archived && existing.Status is not OrchestrationStatus.Archived)
            return Failure(
                OrchestrationErrorCodes.LifecycleTransitionInvalid,
                "Only an archived orchestration can be restored.");
        ...
        if (command.Archived)
        {
            ...
            string[] blockers = enabledAgents
                .Where(value => value.PublishedVersions.LastOrDefault()?.Snapshot?.Orchestrations
                    .Any(binding => binding.OrchestrationId == existing.Id) == true)
                .Select(value => value.Code)
                .Take(8)
                .ToArray();
            if (blockers.Length > 0)
                return Failure(
                    OrchestrationErrorCodes.ArchiveBlocked,
                    $"The orchestration is still referenced by Agent(s): {string.Join(", ", blockers)}.");
        }

        OrchestrationDefinition updated = existing with
        {
            Status = command.Archived ? OrchestrationStatus.Archived : OrchestrationStatus.Disabled,
            LogicalRevision = existing.LogicalRevision + 1
        };
