...
public static class AllServicesMiddleware
{
    public static void UseAllServicesMiddle(this IApplicationBuilder app, IServiceCollection _services)
    {
        ...
        //tsDIAutofac.AddRange(Assembly.LoadFrom(Path.Combine(AppContext.BaseDirectory, "EU.Core.Services.dll")).GetTypes().ToList());
        //tsDIAutofac.AddRange(Assembly.LoadFrom(Path.Combine(AppContext.BaseDirectory, "EU.Core.Repository.dll")).GetTypes().ToList());
