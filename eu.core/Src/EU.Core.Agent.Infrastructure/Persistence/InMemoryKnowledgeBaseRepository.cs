    ...
    IKnowledgeRetriever
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        lock (_gate)
        {
            IEnumerable<KnowledgeBaseDefinition> values = query.Status.HasValue
                ? _values.Values.Where(value => value.Status == query.Status.Value)
                : _values.Values.Where(value => value.Status is not KnowledgeBaseStatus.Archived);
            return Task.FromResult(KnowledgeContractCloner.ReadOnly(values
