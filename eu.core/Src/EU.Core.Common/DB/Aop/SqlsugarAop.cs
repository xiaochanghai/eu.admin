...
{
    private static readonly HashSet<string> DataExecutingProperties = new(StringComparer.Ordinal)
    {
        "CreatedBy",
        "CreatedTime",
        "UpdateTime",
    ...
    {
        if (entityInfo?.EntityValue is null || !DataExecutingProperties.Contains(entityInfo.PropertyName))
            return;

        if (entityInfo.EntityValue is RootEntityTkey<Guid> rootEntity && rootEntity.ID == Guid.Empty)
        ...
        if (entityInfo.EntityValue is BaseEntity baseEntity)
        {
            ...
            switch (entityInfo.OperationType)
            {
                ...
                case DataFilterType.InsertByObject:
                    SetBaseEntityInsertDefaultValue(baseEntity, userId);
                    break;
            }
        }
        else
        {
            ...
            switch (entityInfo.OperationType)
            {
                case DataFilterType.InsertByObject:
                    ...
                    var createdTimeValue = dyCreateTime?.GetValue(entityInfo.EntityValue);
                    if (createdTimeValue is null || (createdTimeValue is DateTime createdTime && createdTime == DateTime.MinValue))
    ...
    private static void SetBaseEntityInsertDefaultValue(BaseEntity baseEntity, Guid? userId)
    {
        if (baseEntity.CreatedBy.IsNullOrEmpty())
            baseEntity.CreatedBy = userId;
        if (baseEntity.GroupId.IsNullOrEmpty())
            baseEntity.GroupId = App.User?.GroupId;
        if (baseEntity.CompanyId.IsNullOrEmpty())
            baseEntity.CompanyId = App.User?.CompanyId;
