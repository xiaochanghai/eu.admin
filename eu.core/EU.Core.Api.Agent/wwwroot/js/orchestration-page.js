...
export function createOrchestrationPage({ api, toast }) {
  ...
  const statusFilter = document.querySelector("#orchestrationStatusFilter");
  const saveButton = document.querySelector("#saveOrchestrationButton");
  const publishButton = document.querySelector("#publishOrchestrationButton");
  const statusButton = document.querySelector("#statusOrchestrationButton");
  const archiveButton = document.querySelector("#archiveOrchestrationButton");
  let current = null;
  let dirty = false;
  let agents = [];
  let activeRun = null;
  ...
  async function load() {
    ...
    try {
      const [values, agentValues] = await Promise.all([
  ...
  function setArchivedState() {
    ...
    workbench.querySelectorAll("input, textarea, select").forEach(node => {
      node.disabled = archived;
    });
    for (const button of [saveButton, publishButton,
      document.querySelector("#addOrchestrationNodeButton"),
      document.querySelector("#addOrchestrationEdgeButton")]) button.disabled = archived;
    const startButton = document.querySelector("#startOrchestrationRunButton");
    startButton.disabled = archived || (current && current.status !== "Enabled");
    startButton.title = startButton.disabled ? "只有已启用的编排可以开始运行" : "";
    workbench.querySelectorAll(".flow-node button, .flow-edge button").forEach(button => {
  ...
  function formatArchiveError(error) {
    if (error.errorCode === "ORCHESTRATION_ARCHIVE_BLOCKED") {
      ...
      const references = markerIndex >= 0
        ...
        : "已启用 Agent";
      return `暂时无法归档：${references}仍在使用该编排。请先解除编排绑定或停用引用方，再重新归档。· 错误码：${error.errorCode}`;
    }
  ...
  async function save(statusOverride = null) {
    try {
      ...
      current = orchestrationDefinitionPresentation(await api.saveOrchestration(current.id, {
        ...
        name: document.querySelector("#orchestrationNameInput").value.trim(),
        description: document.querySelector("#orchestrationDescriptionInput").value,
        status: statusOverride || document.querySelector("#orchestrationStatusInput").value,
  ...
  async function setArchived() {
    ...
    try {
      ...
      dirty = false;
      setArchivedState();
      setText(message, restoring ? "编排已恢复为停用状态。" : "编排已归档。");
      message.dataset.tone = "success";
      await load();
    } catch (error) {
  ...
  function renderRun(value) {
    ...
    document.querySelector("#startOrchestrationRunButton").disabled =
      running || current?.status !== "Enabled";
  }
  ...
  document.querySelector("#addOrchestrationNodeButton").addEventListener("click", () => { addNode(); dirty = true; });
  document.querySelector("#addOrchestrationEdgeButton").addEventListener("click", () => { addEdge(); dirty = true; });
  saveButton.addEventListener("click", () => save());
  publishButton.addEventListener("click", publish);
  statusButton.addEventListener("click", setStatus);
  archiveButton.addEventListener("click", setArchived);
  statusFilter.addEventListener("change", load);
  workbench.addEventListener("input", () => { if (current) dirty = true; });
