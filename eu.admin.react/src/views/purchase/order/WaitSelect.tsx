...
const { TabPane } = Tabs;
const moduleCode = "PO_ORDER_WAIT_NOTICE_MNG";
const moduleCode1 = "PO_ORDER_WAIT_IN_MNG";
const WaitShipSelect: React.FC<any> = props => {
  ...
  let moduleInfo = moduleInfos[waitShipSelectType == "Notice" ? moduleCode : moduleCode1] as ModuleInfo;
  ...
  const okHandle = async () => {
    ...
    selectList1.forEach(record => {
      if (waitShipSelectType == "Notice") record.NoticeQTY = QTYObject[record.ID] ?? 1;
      else record.InQTY = QTYObject[record.ID] ?? 1;
  ...
  hasColumns.push(
    {
      title: waitShipSelectType == "Notice" ? "到货通知数量" : "入库数量",
      dataIndex: waitShipSelectType == "Notice" ? "NoticeQTY" : "InQTY",
      align: "center",
      width: 120,
      render: (_text, record) => (
        <InputNumber
        ...
        <TabPane tab="待选" key="1">
          <ProTable
            ...
            request={async (params, sorter, filterCondition) => {
              // filterCondition["PoOrderId"] = selectedRowIds;
              let filter = { ...filterCondition, PoOrderId: selectedRowIds };
              return await query({
                paramData: JSON.stringify(params),
                sorter: JSON.stringify(sorter),
                filter: JSON.stringify(filter),
                moduleCode: waitShipSelectType == "Notice" ? moduleCode : moduleCode1
              });
            }}
            options={{
              fullScreen: false,
