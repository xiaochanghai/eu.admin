...
import { QRCode, Flex, Form, TreeSelect, Tabs } from "antd";
  ...
  let items: any[] = [
    {
      ...
      key: 3,
      label: "设备图片",
      children: <UploadImage Id={id} isUnique={false} filePath="equipment" />
    },
    {
      key: 4,
      label: "设备附件",
      children: <Attachment Id={id} accept=".png,.jpeg,.pdf,.docx,.xlsx,.zip" filePath="equipment_attachment" />
    },
    {
      key: 5,
      label: "二维码",
      children: id ? <QRCode value={`Equip_${id}`} /> : <>保存数据后查看</>
    }
  ];
  return (
    <>
