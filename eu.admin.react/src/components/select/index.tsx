        ...
        <div className="cursor-pointer" key={`group_${index}`}>
          <div
            className={`flex py-1 my-1 rounded-md item-center   ${classNames({
              " !bg-orange-100 text-orange-400": groupSelected?.value === d.value,
          ...
          >
            ...
            {$nbmp !== 2 && $nbmp !== 1 && <i className=" text-lg icon-subprocess" />}
            ...
            <div className=" w-1/2">
              <div
                ...
                className={`flex p-2  cursor-pointer rounded-md item-center   ${classNames({
                  " !bg-orange-100 text-orange-400": groupSelected === undefined,
          ...
          <div className=" w-1/2">
            {
              <div className=" flex justify-between">
                <div className="font-bold items-start">{groupSelected?.label || `全部${label}`}</div>
                <div className="items-end">{/* <Checkbox onChange={() => {}} /> */}</div>
              </div>
            }
            {selectData &&
              selectData.length > 0 &&
              selectData.map((d: ISelect) => (
                <div key={`select_${d.value}`} className={`flex justify-between p-1 transition-all duration-200 transform`}>
                      ...
                      onChange={() => {
                        if (multiple) {
                          if (value?.map(v => v.value).includes(d.value)) {
                            onDataChange?.(value?.filter(v => v.value !== d.value));
                          } else if (value) {
            ...
            <div className="  space-x-2 flex flex-wrap">
              {value?.map(v => {
                return (
                  <Tag
                    onClose={() => {
                      onDataChange?.(value?.filter(rm => v.value !== rm.value));
                    }}
                    closable={true}
                  >
                    {v.label}
