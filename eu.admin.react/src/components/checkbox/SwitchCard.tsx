...
export default (props: SwitchCardProps) => {
  const { divider, icon, iconNode, title, placeholder, value, onDataChange, component = "switch" } = props;
  return (
    <div className=" w-full flex  items-center    ">
      <div className="w-10 pt-2">
        {iconNode && iconNode}
        {!iconNode && icon && <Icon name={icon}></Icon>}
        {iconNode === undefined && icon === undefined && <i className=" text-3xl icon-supervised_user_circle" />}
      </div>
      <div
      ...
      >
        <div className="flex justify-between">
          ...
          {component === "switch" && <Switch className="justify-end" checked={value} onChange={onDataChange} />}
          {component === "checkbox" && (
            <Checkbox
