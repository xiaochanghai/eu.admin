...
import { appendCom, moveCom, setFocus, removeCom, FieldNodeSchema } from "@/redux/modules/formDesign";
...
export default function Item({ data, parentId, index }: Props) {
  ...
  const className = cl("canvas-field p-2 border border-gray-300 border-dashed  relative", {
    "opacity-2": isDragging,
    "outline-blue border-opacity-0": state.focusId === data.id,
    "inline-block": ["a", "span", "button", "b", "i"].includes(current.type.name)
  });
  ...
  const action = (
    ...
    >
      <svg className="w-4 h-4 bg" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        <path
          strokeLinecap="round"
  ...
  return (
    <div onClick={handleFocus} className={className} ref={ref}>
      ...
      {isOver && canDrop && !positionDown ? <div className="border-indigo-600 border" /> : null}
      <CurrentTag {...data.props}>
        {!isParentNode(data.type)
          ? data.props.children
          : data.children && data.children.map((sub, index) => <Item parentId={data.id} index={index} data={sub} key={sub.id} />)}
      </CurrentTag>
      {isOver && canDrop && positionDown ? <div className="border-indigo-600 border" /> : null}
    </div>
  );
