...
export default function ApiModal({ onChange }: Props): ReactElement {
  ...
  const handleGetApiFn = () => {
    ...
    if (api) {
      fetch(api, {
        cache: "no-cache",
        headers: Object.assign({ "content-type": "application/json" }, header ? JSON.parse(header) : {}),
        method: "GET",
        mode: "cors"
      })
        .then(res => res.json())
        ...
        <div>
          ...
          {apiResult && (
            <>
              ...
              <Form.Item>
                <Input placeholder="设置数据源字段" onChange={e => handleApiField("dataField", e.target.value)} />
              </Form.Item>
              <p style={{ color: "red" }}>数据源字段是接口返回的图表数据对应的字段, 必填, 否则无法正确导入数据</p>
            </>
          )}
        </div>
      </Modal>
